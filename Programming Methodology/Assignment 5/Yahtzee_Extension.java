...
public class Yahtzee_Extension extends GraphicsProgram implements YahtzeeConstants {

	private static final int DELAY = 1500;

	public static void main(String[] args) {
		new Yahtzee_Extension().start(args);
	}
	...
	public void run() {
		...
		if (nPlayers == 1) {
			...
			playerNames[0] = dialog.readLine("Enter name for player 1 ");
	...
	private void playGame() {
		...
		ArrayList<Integer> computerArr = new ArrayList<>();
		for (int j = 0; j < N_SCORING_CATEGORIES; j++) {
			for (int i = 1; i <= nPlayers; i++) {
				int category;
				if (computerMode && i == 2) {
					category = rollForComputer(computerArr);
				} else {
				...
				if (computerMode && i == 2) {
					display.printMessage("Computer has chosen " + getCategory(category));
					pause(2 * DELAY);
				}
			}
		}
	...
	// computer rolls dices
	private int rollForComputer(ArrayList<Integer> computerArr) {
		display.printMessage("Computer's turn!");
		pause(DELAY);
		dices = randomizeDices();
		display.displayDice(dices);
		for (int k = 0; k < SELECT_CHANCE; k++) {
			reRollDices(computerArr);
			display.displayDice(dices);
		}
		int category = getMaxScoreCategory(computerArr);
		computerArr.add(category);
		return category;

	}

	// computer chooses the category from which it takes maximum point each time
	private int getMaxScoreCategory(ArrayList<Integer> computerArr) {
		int maxScore = -1;
		int result = -1;
		for (int category = ONES; category <= CHANCE; category++) {
			...
			int checkCategory = checkCategory(category);
			if (!computerArr.contains(category) && checkCategory > maxScore) {
				maxScore = checkCategory(category);
				result = category;
			}
		}
		return result;
	...
	// this method return category string
	...
	// computer re-rolls dices
	private void reRollDices(ArrayList<Integer> computerArr) {
		int[] frequency = frequency();
		if (!computerArr.contains(THREE_OF_A_KIND) && checkKind(3, frequency)) {
			return;
		} else if (!computerArr.contains(FOUR_OF_A_KIND) && checkKind(4, frequency)) {
			return;
		} else if (!computerArr.contains(FULL_HOUSE) && checkFullHouse(frequency)) {
			return;
		} else if (!computerArr.contains(SMALL_STRAIGHT) && checkStraight(4, frequency)) {
			return;
		} else if (!computerArr.contains(LARGE_STRAIGHT) && checkStraight(5, frequency)) {
			return;
		} else if (!computerArr.contains(YAHTZEE) && checkKind(5, frequency)) {
			return;
		} else {
			pause(DELAY);
			reRollComputer(computerArr);
		}
	}

	// computer re-rolls dices and it's main goal is to roll YAHTZEE combination
	private void reRollComputer(ArrayList<Integer> computerArr) {
		int mostOccured = getMostOccurredDice();
		for (int i = 0; i < dices.length; i++) {
			if (dices[i] != mostOccured) {
				dices[i] = rgen.nextInt(1, 6);
			}
		}
	}

	// this method returns most occurred dice
	private int getMostOccurredDice() {
	...
	private void endGame(int[][] totalScore) {
		...
		updateTop10(totalScore);
	}

	// this method updates top 10 high scores if any of the player earns more points
	private void updateTop10(int[][] totalScore) {
		ArrayList<String> top10 = getTop10();
		updateTop10(top10, totalScore);
		updateFile(top10);
	}

	// this method updates top 10 in file
	private void updateFile(ArrayList<String> top10) {
		try {
			BufferedWriter br = new BufferedWriter(new FileWriter("TOP 10.txt"));
			for (int i = 0; i < top10.size(); i++) {
				br.write(top10.get(i));
				br.newLine();
			}
			br.close();
		} catch (Exception ex) {
	...
	// updating top 10
	private void updateTop10(ArrayList<String> top10, int[][] totalScore) {
		for (int i = 0; i < totalScore.length; i++) {
			for (int j = 0; j < top10.size(); j++) {
				int highscore = Integer.parseInt(top10.get(j).substring(top10.get(j).indexOf(' ') + 1));
				if (totalScore[i][2] > highscore) {
					top10.add(j, playerNames[i] + " " + Integer.toString(totalScore[i][2]));
	...
	// this method reads file and gives top 10 players and scores
	private ArrayList<String> getTop10() {
		ArrayList<String> strArr = new ArrayList<>();
		try {
			BufferedReader rd = new BufferedReader(new FileReader("TOP 10.txt"));
			while (true) {
