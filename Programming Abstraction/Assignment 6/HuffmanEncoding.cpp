...
Node* buildEncodingTree(Map<ext_char, int>& frequencies) {
	...
	foreach(ext_char ch in frequencies) {
		...
		newNode->one = NULL;
		newNode->zero = NULL;
		nodes.enqueue(newNode, newNode->weight);
	}

	while(nodes.size() > 1) {
		Node* first = nodes.dequeue();
		Node* second = nodes.dequeue();
		Node* newNode = new Node;
		newNode->zero = first;
		newNode->one = second;
 ...
 */ 
void getEncodedCharacters(Node* encodingTree, Map<ext_char, string> &characters, 
						  string cur) {
	if(!encodingTree) return;
	if(encodingTree->character != NOT_A_CHAR) {
		characters[encodingTree->character] = cur;
	} else {
		getEncodedCharacters(encodingTree->zero, characters, cur + '0');
		getEncodedCharacters(encodingTree->one, characters, cur + '1');
	}
}
...
void encodeFile(istream& infile, Node* encodingTree, obstream& outfile) {
	...
	string cur;
	getEncodedCharacters(encodingTree, characters, cur);
	char ch;
	while(infile.get(ch)) {
		string temp = characters[ch];
		for(int i = 0; i < temp.length(); i++) {
			outfile.writeBit(temp[i] - '0');
		}
	}
	string temp = characters[PSEUDO_EOF];
	for(int i = 0; i < temp.length(); i++) {
		outfile.writeBit(temp[i] - '0');
	}
}
...
void decodeFile(ibstream& infile, Node* encodingTree, ostream& file) {
	...
	while(true) {
		if(temp->character == PSEUDO_EOF) break;
		if(temp->character != NOT_A_CHAR) {
			file << char(temp->character);
			temp = encodingTree;
		}
