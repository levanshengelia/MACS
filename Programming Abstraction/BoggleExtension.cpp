 ...
 * Exstensions:
 * 5x5 board;
 * "QU" instead of "Q", making Q useful letter;
 * If a player finds longest possible word, he/she gets 20 extra points;
 */
#include <iostream>
#include "gboggle.h"
#include "grid.h"
...
void play();
void set_game(Grid<string> & cubes);
void set_configuration(Grid<string> & cubes, int size);
void set_custom_configuration(Grid<string> & cubes, int size);
void set_default_configuration(Grid<string> & cubes, int size);
void draw_cubes(Grid<string> & cubes);
void player_move(Grid<string> & cubes, Lexicon & lexicon, string longest);
void fill_grid(string configuration, Grid<string> & cubes);
bool is_valid_guess(Grid<string> & cubes, string guess, Set<pair<int, int> > & visited,
					Lexicon & lexicon, int row, int col, int index);
void computer_move(Grid<string> & cubes, Lexicon & lexicon, Set<string> & foundWords);
void find_words(Grid<string> & cubes, Set<string> & foundWords, int row, int col, 
				Set<pair<int, int> > & visited, Lexicon & lexicon, string word);
void highlight(Grid<string> & cubes, Set<pair<int, int> > & visited);
string get_longest_word(Set<string> & foundWord);
...
void play() {
	...
	computer_move(cubes, lexicon, foundWords);
	string longest = get_longest_word(foundWords);
	player_move(cubes, lexicon, longest);
...
void fill_grid(string configuration, Grid<string> & cubes) {
	...
	for(int i = 0; i < cubes.nRows; i++) {
		for(int j = 0; j < cubes.nCols; j++) {
			string str = "";
			str += configuration[index];
			if(configuration[index] == 'Q') str += 'U';
			cubes[i][j] = str;
			index++;
		}
	}
...
void draw_cubes(Grid<string> & cubes) {
	for(int row = 0; row < cubes.nRows; row++) {
		for(int col = 0; col < cubes.nCols; col++) {
			labelCube(row, col, cubes[row][col][0]);
		}
	}
}
...
void player_move(Grid<string> & cubes, Lexicon & lexicon, string longest) {
	...
	while(true) {
		...
		if(guess.length() < 4) {
			...
			for(int i = 0; i < cubes.nRows; i++) {
				for(int j = 0; j < cubes.nCols; j++) {
					if(cubes[i][j][0] == guess[0]) {
						visited.clear();
						if((guess[0] == 'Q' && guess[1] == 'U' && 
							is_valid_guess(cubes, guess, visited, lexicon, i, j, 2)) ||
							is_valid_guess(cubes, guess, visited, lexicon, i, j, 1)) {
							recordWordForPlayer(guess, HUMAN);
							if(guess == longest) {
								recordWordForPlayer("BONUS*", HUMAN);
								cout << "You found the longest possible word! +20 bonus points!" << endl;
							}
		...
		for(int dRow = -1; dRow <= 1; dRow++) {
			for(int dCol = -1; dCol <= 1; dCol++) {
				...
				if(cubes.inBounds(location.first, location.second) && !visited.contains(location)
					&& cubes[location.first][location.second][0] == guess[index]
					&& cubes[location.first][location.second][0] == 'Q' 
					&& guess[index + 1] == 'U' 
					&& is_valid_guess(cubes, guess, visited, lexicon, location.first, 
					location.second, index + 2)) {
					return true;
				}
				if(cubes.inBounds(location.first, location.second) && !visited.contains(location)
					&& cubes[location.first][location.second][0] == guess[index]
...
* Usage: computer_move(cubes, lexicon, foundWords);
* ----------------------------
...
* Usage: get_longest_word(foundWord);
* -----------------------------------
* This function finds the longest word in set
*/
string get_longest_word(Set<string> & foundWord) {
	string res = "";
	foreach(string word in foundWord) {
