...
(define (intersection-points circles) 
  (if
    (null? (cdr circles)) '()
    (append
      (apply append (map (lambda(arg) (intersect (car circles) arg)) (cdr circles)))
      (intersection-points (cdr circles))
    )
  )
)

(define (distance-product startingPoint destinations)
  (apply *
    (map 
      (lambda (arg) (dist startingPoint arg))
      (remove startingPoint destinations)
    )
  )
)
...
(define (rate-points points)
  (map
    (lambda (arg) (list (distance-product arg points) arg))
    points
  )
)
...
(define (clumped-points points)
  (map
    cadr
    (prefix-of-list (sort-points (rate-points points)) (quotient (length points) 2))
  )
)

(define (average-x points)
  (/ (apply + 
        (map
          car
          points
...
(define (average-point points)
  (let
    (
    (avgX (average-x points))
    (avgY (average-y points))
    )
    (list (distance-product (list avgX avgY) points) (list avgX avgY))
  )
)
...
(define (where-am-i distances points)
  (sort-points
    (map
      best-estimate
      (all-guesses distances points)
    )
  )
)
