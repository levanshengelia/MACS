...
void generate_sentence(map<string, Definition>& grammar, string word, vector<string> &sentence) {
  Definition start = grammar[word];
  Production first_production = start.getRandomProduction();
  for(Production::iterator it = first_production.begin(); it < first_production.end(); it++) {
    string word = *it;
    if(word[0] == '<') generate_sentence(grammar, word, sentence);
    else sentence.push_back(word);
  }
}

// printing the sentence
void print_sentence(vector<string> &sentence) {
  for(long unsigned int i = 0; i < sentence.size(); i++) {
    cout << sentence[i];
    if(i != sentence.size() - 1 && sentence[i + 1] == ".") continue;
    cout << " ";
...
int main(int argc, char *argv[])
{
  ...
  for(int i = 1; i <= 3; i++) {
    ...
    cout << "Version #" << i << ": ---------------------" << endl;
    cout << " ";
    generate_sentence(grammar, "<start>", sentence);
